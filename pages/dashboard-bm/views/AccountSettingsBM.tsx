...
const AccountSettingsBM: React.FC<AccountSettingsBMProps> = ({ organizationData, onUpdatePhoneNumber, onUpdateProfilePicture, onUpdatePassword }) => {
    ...
    <div>
      ...
      <div className="relative">
        ...
        <input type={isShown ? "text" : "password"} id={id} value={value} onChange={(e) => setter(e.target.value)} required={!disabled} disabled={disabled}
               className={`w-full py-2.5 pl-10 pr-10 border rounded-lg shadow-sm focus:ring-1 focus:ring-hegra-turquoise transition-colors text-sm bg-white ${error ? 'border-red-500' : 'border-gray-300'} ${disabled ? 'text-gray-400 cursor-not-allowed' : ''}`} placeholder={label}/>
        <button type="button" onClick={showToggler} className="absolute inset-y-0 right-0 pr-3 flex items-center text-gray-500 hover:text-hegra-navy" disabled={disabled}>{isShown ? <EyeOff size={16}/> : <Eye size={16}/>}</button>
      </div>
  ...
  return (
    <div className="p-0 sm:p-2 md:p-4 bg-gray-50 min-h-full">
      <h1 className="text-2xl sm:text-3xl font-bold text-hegra-deep-navy mb-8">Pengaturan Akun Organisasi</h1>
      <div className="bg-white p-6 rounded-lg shadow-md border border-gray-200">
        <div className="text-center mb-8">
          <div className="relative w-36 h-36 rounded-full mx-auto mb-3 group">
            <img src={profilePicturePreview || `https://ui-avatars.com/api/?name=${encodeURIComponent(organizationData.organizationName)}&background=18093B&color=FFFFFF&size=144`} alt="Logo Organisasi" className="w-full h-full rounded-full object-cover border-4 border-hegra-turquoise/30"/>
          ...
          <h2 className="text-xl font-semibold text-hegra-navy mt-3">{organizationData.organizationName}</h2>
        ...
        <div className="space-y-6 max-w-2xl mx-auto">
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              <div className="relative flex-shrink-0">
                 <select id="countryCode" name="countryCode" value={selectedCountryCode} onChange={(e) => setSelectedCountryCode(e.target.value)} disabled={isPhoneNumberDisabled} className="appearance-none z-10 h-full block w-auto py-2.5 pl-3 pr-8 text-sm border rounded-l-lg focus:ring-hegra-turquoise focus:border-hegra-turquoise bg-white disabled:bg-white disabled:text-gray-400 disabled:cursor-not-allowed border-gray-300">
          ...
          {showPasswordForm && (
            <form onSubmit={handlePasswordSubmit} className="space-y-4 mt-4 pt-4 border-t border-gray-100">
              ...
              <div className="flex flex-col sm:flex-row gap-3"><button type="submit" className="w-full sm:w-auto flex items-center justify-center gap-2 bg-hegra-yellow text-hegra-navy font-bold py-2.5 px-4 rounded-lg hover:bg-opacity-90 transition-colors text-sm"><Save size={16}/> Simpan Password Baru</button><button type="button" onClick={() => { setShowPasswordForm(false); setOldPassword(''); setNewPassword(''); setConfirmNewPassword(''); setPasswordErrors({});}} className="w-full sm:w-auto text-sm text-gray-600 hover:text-gray-800 py-2.5 px-4 rounded-lg border border-gray-300 hover:bg-gray-100">Batal</button></div>
            </form>
          )}
        </div>
      </div>
      {showConfirmModal && (<ConfirmationModal isOpen={showConfirmModal} title="Konfirmasi Nomor Telepon" message={`Apakah Anda yakin nomor telepon ${phoneNumberToConfirm} sudah benar?`} confirmText="Ya, Simpan" cancelText="Batal" onConfirm={confirmPhoneNumberSave} onCancel={() => setShowConfirmModal(false)} icon={Info} iconColorClass="text-hegra-turquoise" confirmButtonClass="bg-hegra-turquoise hover:bg-opacity-90 text-white"/>)}
    </div>
  );
};
